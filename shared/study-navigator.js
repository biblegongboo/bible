(function(){
  ...
  function mount(){var title=document.querySelector('.sat-title'),logo=document.querySelector('.sat-logo');if(!title||!logo)return;var host=document.createElement('div');host.className='study-nav-host';logo.insertBefore(host,title);title.classList.add('study-nav-legacy-title');button=document.createElement('button');button.type='button';button.className='study-nav-button';button.setAttribute('aria-haspopup','dialog');button.setAttribute('aria-expanded','false');button.onclick=open;restoreButton(host);
    ...
    new MutationObserver(function(){
      if(button.parentNode!==host||!button.querySelector('.study-nav-label')||!button.querySelector('.study-nav-arrow'))restoreButton(host);
